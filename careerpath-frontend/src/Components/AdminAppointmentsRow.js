...
import { bookAppointment, deleteAppointment, getSlots, getUserWithId } from '../apis';
import moment from 'moment';
import LoadingIndicator from './LoadingIndicator';
import DatePicker from 'react-date-picker';
Modal.setAppElement('#root');
export default function Appointmentrow({ title, coach, image, studentId, coachId, appointment, slot }) {
  ...
  const fetchStudent = (studentId) => {
    setloading(true);
    getUserWithId(studentId).then((res) => {
      if (res.success) {
        setstudent(res.user);
        setloading(false);
      } else {
  ...
  return (
    <div className='row no-gutters border-bottom'>
      ...
      <div style={{ height: '140px', textAlign: 'justify' }} className='col small'>
        {student ? (
          <>
            <p className='mb-1'>Student Name: {student.name || student.username}</p>
            <p className='mb-1'>Coach Name: {coach.name || coach.username}</p>
            <p className='mb-1'>Schedule Time: {moment(slot.start).format('MMMM Do YYYY, h:mm:ss a')}</p>
          </>
        ) : (
      ...
      <LoadingIndicator isLoading={deleting}>
        <div className='col m-2'>
          ...
          <button
            ...
            className='btn logout-btn logout-btn-appointment my-2 mx-4 small'
          >
            Cancel Appointment
          </button>
        </div>
